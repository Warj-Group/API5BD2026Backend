...
def get_consumo_materiais(db: Session) -> List[FactConsumoMateriais]:
    ...
    return db.query(FactConsumoMateriais).order_by(FactConsumoMateriais.id_fato_material).all()
...
def get_consumo_by_id(db: Session, fato_id: int) -> Optional[FactConsumoMateriais]:
    ...
    return db.query(FactConsumoMateriais).filter(
        FactConsumoMateriais.id_fato_material == fato_id
    ).first()
...
def get_consumo_by_projeto(db: Session, projeto_id: int) -> List[FactConsumoMateriais]:
    ...
    return db.query(FactConsumoMateriais).filter(
        FactConsumoMateriais.projeto_id == projeto_id
    ).all()


def get_consumo_by_material(db: Session, material_id: int) -> List[FactConsumoMateriais]:
