...
def get_horas_trabalhadas(db: Session) -> List[FactHorasTrabalhadas]:
    ...
    return (
        db.query(FactHorasTrabalhadas)
        .order_by(FactHorasTrabalhadas.id_fato_horas)
        .all()
    )
...
def get_horas_by_id(db: Session, fato_id: int) -> Optional[FactHorasTrabalhadas]:
    ...
    return (
        db.query(FactHorasTrabalhadas)
        .filter(FactHorasTrabalhadas.id_fato_horas == fato_id)
        .first()
    )
...
def get_horas_by_programa(db: Session, programa_id: int) -> List[FactHorasTrabalhadas]:
    ...
    return (
        db.query(FactHorasTrabalhadas)
        .filter(FactHorasTrabalhadas.programa_id == programa_id)
        .all()
    )
...
def get_horas_by_projeto(db: Session, projeto_id: int) -> List[FactHorasTrabalhadas]:
    ...
    return (
        db.query(FactHorasTrabalhadas)
        .filter(FactHorasTrabalhadas.projeto_id == projeto_id)
        .all()
    )
...
def get_horas_by_usuario(db: Session, usuario_id: int) -> List[FactHorasTrabalhadas]:
    ...
    return (
        db.query(FactHorasTrabalhadas)
        .filter(FactHorasTrabalhadas.usuario_id == usuario_id)
        .all()
    )
...
def get_horas_by_tarefa(db: Session, tarefa_id: int) -> List[FactHorasTrabalhadas]:
    ...
    return (
        db.query(FactHorasTrabalhadas)
        .filter(FactHorasTrabalhadas.tarefa_id == tarefa_id)
        .all()
    )
...
def get_total_horas_by_projeto(db: Session, projeto_id: int) -> float:
    ...
    """
    resultado = (
        db.query(func.sum(FactHorasTrabalhadas.horas_trabalhadas))
        .filter(FactHorasTrabalhadas.projeto_id == projeto_id)
        .scalar()
    )
...
def get_custo_total_by_projeto(db: Session, projeto_id: int) -> float:
    ...
    """
    resultado = (
        db.query(func.sum(FactHorasTrabalhadas.custo_total))
        .filter(FactHorasTrabalhadas.projeto_id == projeto_id)
        .scalar()
    )
    return resultado or 0.0
